...
describe('Models/Worker', function () {
  ...
  it('#getConcurrency() should throw error if no worker assigned to passed in job name.', async () => {
    ...
    try {
      ...
      throw new Error(
        'getConcurrency() should have thrown an error due to no worker assigned to that job name.',
      )
    } catch (error) {
      error.should.deepEqual(
        new Error('Job ' + jobName + ' does not have a worker assigned to it.'),
      )
    }
  })
  ...
  it('#executeJob() should error if worker not assigned to job yet.', async () => {
    ...
    try {
      ...
      throw new Error(
        'execute job should have thrown an error due to no worker assigned to that job name.',
      )
    } catch (error) {
      error.should.deepEqual(
        new Error('Job ' + jobName + ' does not have a worker assigned to it.'),
  ...
  it('#executeJob() timeout logic should work if timeout is set.', async () => {
    ...
    try {
      ...
      error.should.deepEqual(
        new Error('TIMEOUT: Job id: ' + job.id + ' timed out in ' + jobTimeout + 'ms.'),
      )
    }
  })
  ...
  it('#executeJobLifecycleCallback() job lifecycle callbacks that error out should gracefully degrade to console error.', async () => {
    ...
    // Cache console error.
    const consoleErrorCache = console.error
