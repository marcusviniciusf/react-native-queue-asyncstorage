export default class LegacyAsyncStorage {
  getSingle(key, opts) {
    return asMock.getItem(key)
  }
  setSingle(key, value, opts) {
    return asMock.setItem(key, value)
  }
  getMany(keys, opts) {
    return asMock.multiGet(keys)
  }
  setMany(values, opts) {
    return asMock.multiSet(values)
  }
  removeSingle(key, opts) {
    return asMock.removeItem(key)
  }
  removeMany(keys, opts) {
    return asMock.multiRemove(keys)
  }
  getKeys(opts) {
    return asMock.getAllKeys()
  }
  dropStorage(opts) {
    return asMock.clear()
  }
...
async function _multiGet(keys, callback) {
  const values = keys.reduce((map, key) => {
    map[key] = asMock.__INTERNAL_MOCK_STORAGE__[key]
    return map
  }, {})
