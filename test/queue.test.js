...
describe("Models/Queue", function() {
  ...
  it("#start(lifespan) queue with lifespan does not process jobs that have no timeout set.", async () => {
    ...
    queue.addWorker(name, () => {})

    // Create a couple jobs
    queue.createJob(name, {}, {}, false)
    queue.createJob(name, {}, {}, false)
    queue.createJob(name, {}, {}, false)
  ...
  it("#start(lifespan) BASIC TEST (One job type, default job/worker options): queue will process jobs with timeout set as expected until lifespan ends.", async () => {
    ...
    // Start queue, don't await so this test can continue while queue processes.
    queue.start(queueLifespan)

    // Wait 1 second to ensure that the job has finished processing before checking.
    setTimeout(async () => {
      ...
      if (badJobs.length) {
        throw new Error(
          "ERROR: Queue with lifespan picked up bad jobs it did not have enough remaining lifespan to execute: " +
            JSON.stringify(badJobs),
        )
      }
  ...
  it('#start(lifespan) "Zero lifespanRemaining" edge case #2 is properly handled.', async () => {
    ...
    await new Promise(resolve => {
      setTimeout(resolve, 25)
    }) // Space out inserts so time sorting is deterministic.
  ...
  it("QueueFactory initializes database", async () => {
    const queue = await QueueFactory()
    const emptyArray = []

    queue.database.database.should.be.deepEqual(emptyArray)
  })

  it("init() Calling init() multiple times will only set queue.database once.", async () => {
    const queue = await QueueFactory()

    queue.database.database.should.deepEqual([])

    queue.database = ["arbitrary-string"]

    queue.init()

    queue.database.should.not.equal("arbitrary-string")
  })
  ...
  it("#start() called when queue is already active should NOT fire up a concurrent queue.", async () => {
    ...
    await queue.addWorker(jobName, async () => {
  ...
  it("#getConcurrentJobs() If worker concurrency is set to 3, getConcurrentJobs() should get up to 3 of same type of job as next job on top of queue.", async () => {
    ...
    await queue.addWorker("a-different-job", () => {
      return true
    })
  ...
  it("#getConcurrentJobs() consecutive calls to getConcurrentJobs() gets new non-active jobs (and marks them active).", async () => {
    ...
    JSON.parse(moreConcurrentJobs[0].payload).should.deepEqual({
      random: "this is 2nd random data",
    })
    JSON.parse(moreConcurrentJobs[1].payload).should.deepEqual({
      random: "this is 1st random data",
    })
    JSON.parse(moreConcurrentJobs[2].payload).should.deepEqual({
      random: "this is 3rd random data",
    })

    // Now we should have 4 active jobs...
    const allJobsAgain = await queue.getJobs()
    ...
    JSON.parse(fifthConcurrentJobs[0].payload).should.deepEqual({
      random: "this is 4th random data",
    })
  ...
  it("#processJob() increments failedAttempts counter until max attempts then fails on job failure.", async () => {
    ...
    await queue.createJob(
      jobName,
      { random: "this is 2nd random data" },
      { priority: 1, timeout: 3000, attempts: 3 },
      false,
    )
    await queue.createJob(
      "a-different-job",
    ...
    // Process the jobs
    await Promise.all([
      queue.processJob(secondConcurrentJobs[0]),
      queue.processJob(secondConcurrentJobs[1]),
    ])
    ...
    const secondJobs = await queue.getJobs()
    ...
    // Ensure other job was deleted on job completion and ensure failedAttempts incremented again on failed job.
    const thirdJobs = await queue.getJobs()
  ...
  it("#processJob() logs errors on job failure", async () => {
    ...
    const logCheckOneJob = await queue.getJobs()

    logCheckOneJob[0].data.should.equal(
      JSON.stringify({
        attempts: 3,
        failedAttempts: 1,
    ...
    const logCheckTwoJob = await queue.getJobs()

    logCheckTwoJob[0].data.should.equal(
      JSON.stringify({
        attempts: 3,
        failedAttempts: 2,
    ...
    const logCheckThreeJob = await queue.getJobs()

    logCheckThreeJob[0].data.should.equal(
      JSON.stringify({
        attempts: 3,
        failedAttempts: 3,
  ...
  it("onComplete fires only once on job with multiple attempts that ends in success.", async () => {
    ...
    await queue.createJob(
      jobName,
      {
        random: "this is 1st random data succes",
      },
      {
        attempts,
      },
